...
class User(UserMixin, db.Model):
    ...
    # This will act like a list if BlogPost object attached to each User
...
class Comment(db.Model):
    ...
    # "user.id" The users refers to the tablename of the User class
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Login and authenticate user after details to database
        login_user(new_user)
...
def login():
    ...
    if form.validate_on_submit():
        email = form.email.data
        password_submit = form.password.data
        user = User.query.filter_by(email=email).first()
        if not user:
            flash("That email dose not exist, please try again.")
            return redirect(url_for("login"))
        elif not check_password_hash(user.password, password_submit):
